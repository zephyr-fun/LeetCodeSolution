...
class UnionFindSet {
public:
    UnionFindSet(int k, vector<vector<int>>& g) : parent(k + 1), rank(k + 1, 0), size(k + 1, 0) {
        int n = g.size();
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < n; j++) {
                parent[i * n + j] = i * n + j;
    ...
    int largestIsland(vector<vector<int>>& grid) {
        ...
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < n; j++) {
                ...
                res = max(res, set.getSize(getIdx(i, j, n))); // extra cost, you can optim this by delete it
            }
        }
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < n; j++) {
                if(grid[i][j] == 0) {
                    ...
                    for(int k = 0; k < 4; k++) {
                        int ni = i + dirs[k][0];
                        int nj = j + dirs[k][1];
                        if(ni < 0 || ni >= n || nj < 0 || nj >= n || grid[ni][nj] == 0) { // grid[ni][nj] == 0 matters
                            continue;
                        }
                        if(!root_set.count(set.Find(getIdx(ni, nj, n)))) {
                            root_set.insert(set.Find(getIdx(ni, nj, n)));
                            temp += set.getSize(getIdx(ni, nj, n));
                        }
                    }
                    res = max(res, temp);
                }
